...
class MainActivity : BaseActivity(), View.OnClickListener {
    ...
    private var hasTopic: Boolean = false   //课程是否有主题
    private var isSelf: Boolean = false
    private var isNoLimit: Boolean = false
    private var canCreateWS: Boolean = false    //培训是否是创建工作坊考核
    ...
    private fun onEmptyTrain() {
        ...
        val tvCreateWs = findViewById<TextView>(R.id.tv_createWs)
        val tvCmtsLearn = findViewById<TextView>(R.id.tv_cmtsLearn)
        tvCreateWs.setOnClickListener { startActivity(Intent(context, WorkshopEditActivity::class.java)) }
    ...
    private fun updateCourseListUI(list: List<MyTrainInfo.CourseRegisters>) {
        ...
        if (courseDatas.isNotEmpty()) {
            ...
            val llCourse = findViewById<LinearLayout>(R.id.ll_selectCourse)
            if (isNoLimit) {
                llCourse.visibility = View.VISIBLE
                llCourse.setOnClickListener { openCourseRegister() }
            } else {
                llCourse.visibility = View.GONE
            }
        } else {
            rvCourse.visibility = View.GONE
            llEmpty.visibility = View.VISIBLE
            val btSelect = findViewById<Button>(R.id.bt_selectCourse)
            if (isSelf) {
                btSelect.visibility = View.VISIBLE
                btSelect.setOnClickListener { openCourseRegister() }
            } else {
                btSelect.visibility = View.GONE
            }
        }
    }

    private fun openCourseRegister() {
        val intent = Intent(context, RegistCouseActivity::class.java)
    ...
    private fun updateWorkShopListUI(mDatas: List<WorkShopMobileUser>) {
        ...
        if (mDatas.isNotEmpty()) {
            ...
            llEmptyWS.visibility = View.GONE
        } else {
            rvWorkshop.visibility = View.GONE
            llEmptyWS.visibility = View.VISIBLE
            val tvWsConfig = findViewById<TextView>(R.id.tv_wsConfig)
            val btCreateWs = findViewById<Button>(R.id.bt_createWs)
            if (canCreateWS) {
                tvWsConfig.text = "暂未创建个人工作坊"
                btCreateWs.visibility = View.VISIBLE
                btCreateWs.setOnClickListener {
                    val intent = Intent(context, WorkshopEditActivity::class.java)
                    intent.putExtra("trainId", trainId)
                    startActivityForResult(intent, 2)
                }
            } else {
                tvWsConfig.text = "需参与工作坊考核"
                btCreateWs.visibility = View.GONE
            }
        }
    }
    ...
    override fun onClick(v: View) {
        when (v.id) {
            ...
            R.id.ll_course -> scrollToPosition(findViewById(R.id.ll_courseLayout))
    ...
    override fun obBusEvent(event: MessageEvent) {
        if (event.action == Action.CHANGE_USER_ICO && event.obj != null && event.obj is String) {
            val avatar = event.obj as String
            menu.menu?.findViewById<ImageView>(R.id.iv_userIco)?.let {
                GlideImgManager.loadCircleImage(applicationContext, avatar, R.drawable.user_default, R.drawable.user_default, it)
            }
        } else if (event.action == Action.CHANGE_USER_NAME && event.obj != null && event.obj is String) {
            val realName = event.obj as String
            menu.menu?.findViewById<TextView>(R.id.tv_userName)?.let { it.text = realName }
        } else if (event.action == Action.CHANGE_DEPT_NAME && event.obj != null && event.obj is String) {
            val deptName = event.obj as String
            menu.menu?.findViewById<TextView>(R.id.tv_deptName)?.let { it.text = deptName }
        } else if (event.action == Action.CHOOSE_COURSE_STATE) {
            mInfoMap.remove(trainId)
            trainId?.let { getTrainInfo(it) }
        }
    }
