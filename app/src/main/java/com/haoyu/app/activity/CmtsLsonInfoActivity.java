...
public class CmtsLsonInfoActivity extends BaseActivity implements View.OnClickListener {
    private CmtsLsonInfoActivity context;
    ...
    public void setLesson() {
        tv_title.setText(lessonEntity.getTitle());
        if (lessonEntity.getCreator() != null && lessonEntity.getCreator().getAvatar() != null) {
            GlideImgManager.loadCircleImage(context, lessonEntity.getCreator().getAvatar(),
                    R.drawable.user_default, R.drawable.user_default, iv_userIco);
        } else {
            iv_userIco.setImageResource(R.drawable.user_default);
        }
        if (lessonEntity.getCreator() != null) {
            tv_userName.setText(lessonEntity.getCreator().getRealName());
        }
        tv_createTime.setText("发布于" + TimeUtil.getSlashDate(lessonEntity.getCreateTime()));
        int browseNum = 0, adviseNum = 0;
        if (lessonEntity.getmDiscussionRelations() != null && lessonEntity.getmDiscussionRelations().size() > 0) {
            browseNum = lessonEntity.getmDiscussionRelations().get(0).getBrowseNum();
            supportNum = lessonEntity.getmDiscussionRelations().get(0).getSupportNum();
            adviseNum = lessonEntity.getmDiscussionRelations().get(0).getReplyNum();
        }
        tv_heatNum.setText("热度（" + browseNum + "）");
        tv_supportNum.setText("赞（" + supportNum + "）");
        tv_adviseNum.setText("提建议（" + adviseNum + "）");
        if (lessonEntity.getContent() != null && lessonEntity.getContent().trim().length() > 0) {
            tv_content.setVisibility(View.VISIBLE);
            Html.ImageGetter imageGetter = new HtmlHttpImageGetter(tv_content, Constants.REFERER, true);
            Spanned spanned = Html.fromHtml(lessonEntity.getContent(), imageGetter, null);
