...
public class AppHomePageActivity extends BaseActivity implements View.OnClickListener {
    ...
    View tv_createWs;   //空培训  创建工作坊组织共同研修
    ...
    View ll_selectCourse;      //当选课限制学时，所选课程未达目标学时，则显示选课按钮
    ...
    private void parseCaptureResult(String result) {
        if (result.startsWith("http") || result.startsWith("https")) {   //如果结果是链接
            ...
            try {
                Gson gson = new Gson();
                if (gson.fromJson(result, Object.class) instanceof CaptureResult) {
