 ...
 * 作者:xiaoma
 */
class CmtsSaysMainFragment : BaseFragment(), CmtsSaysChildFragment.OnResponseListener {
    ...
    override fun initView(view: View) {
        ...
        texts[0] = resources.getString(R.string.study_says_all)
        texts[1] = resources.getString(R.string.study_says_my)
    ...
    private fun setTab(tab: Int) {
        ...
        when (tab) {
            1 -> {
                if (fragments[0] == null) {
                    fragments[0] = CmtsSaysChildFragment()
                    fragments[0]?.let {
                        val bundle = Bundle()
                        bundle.putInt("type", 1)
                        it.arguments = bundle
                        (it as CmtsSaysChildFragment).setOnResponseListener(this)
            ...
            2 -> {
                if (fragments[1] == null) {
                    fragments[1] = CmtsSaysChildFragment()
                    fragments[1]?.let {
                        val bundle = Bundle()
