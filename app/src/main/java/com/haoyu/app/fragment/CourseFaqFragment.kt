 ...
 * 作者:xiaoma
 */
class CourseFaqFragment : BaseFragment(), XRecyclerView.LoadingListener {
    ...
    override fun onAttach(context: Context?) {
        ...
        if (type == 2) baseUrl += "&follow.creator.id=$userId"
        else if (type == 3) baseUrl += "&creator.id=$userId"
    }
    ...
    override fun initView(view: View) {
        ...
        adapter = QuestionAdapter(context, mDatas, 1)
        xRecyclerView.adapter = adapter
        xRecyclerView.setLoadingListener(this)
        when (type) {
            1 -> tvEmpty.text = resources.getString(R.string.empty_questions)
            2 -> tvEmpty.text = resources.getString(R.string.empty_follow)
            else -> tvEmpty.text = resources.getString(R.string.empty_ask)
        }
        registRxBus()
    }
    ...
    override fun setListener() {
        ...
        onTouchListener.setIndependentViews(R.id.tvCollection, R.id.tvBottom).setClickable(object : RecyclerTouchListener.OnRowClickListener {
    ...
    private fun onFollow(position: Int, type: Int) {
        ...
        val map = if (type == 1) HashMap<String, String>().apply {
            ...
            put("followEntity.type", "course_study_question")
