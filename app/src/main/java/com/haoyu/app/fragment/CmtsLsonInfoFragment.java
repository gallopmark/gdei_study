...
public class CmtsLsonInfoFragment extends BaseFragment implements View.OnClickListener {
    ...
    private void updateUI(TeachingLessonEntity mLesson) {
        int remainDay = mLesson.getRemainDay();
        ...
        if (mLesson.getContent() != null && mLesson.getContent().trim().length() > 0) {
            tv_content.setVisibility(View.VISIBLE);
            Html.ImageGetter imageGetter = new HtmlHttpImageGetter(tv_content, Constants.REFERER, true);
            Spanned spanned = Html.fromHtml(mLesson.getContent(), imageGetter, null);
